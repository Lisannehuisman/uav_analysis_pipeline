...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--per-image",
        default=str(PROJECT_ROOT / "results" / "intermediate" / "per_image_metrics_model_b.csv"),
        help="Per-image metrics CSV for one model.",
    )
    parser.add_argument(
        "--per-class-csv",
        default=str(PROJECT_ROOT / "results" / "tables" / "detector_family_per_class_ap50_95.csv"),
    ...
    parser.add_argument(
        "--output-dir",
        default=str(PROJECT_ROOT / "results" / "recomputed" / "factor_level_viewpoint_analysis"),
