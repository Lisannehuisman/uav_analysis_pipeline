...
# Repository root: uav_analysis_pipeline/
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run factor-level viewpoint analysis for the YOLOv8l-M4 detector "
            "using cached per-image evaluation metrics."
        )
    )
    ...
    parser.add_argument(
        ...
        default=str(
            ...
            / "analysis_inputs"
            / "viewpoint"
            / "per_image_metrics_yolov8l_m4.csv"
        ),
        help=(
            "Cached per-image metrics CSV. "
            "Defaults to the YOLOv8l-M4 viewpoint-analysis cache included "
            "under data/analysis_inputs/viewpoint."
        ),
    )
    ...
    parser.add_argument(
        ...
        help=(
            "Per-class AP50-95 CSV used to determine the canonical class names."
        ),
    )
    ...
    parser.add_argument(
        ...
        help="Per-image metric used for the viewpoint analysis.",
    )

    parser.add_argument(
        "--label",
        default="YOLOv8l-M4",
        help="Model label used in generated figures and summaries.",
    )
    ...
    parser.add_argument(
        ...
        help="Directory where tables, figures and the analysis summary are written.",
    )
...
def main() -> None:
    ...
    if not per_image_path.exists():
        raise FileNotFoundError(
            f"Per-image metrics file not found: {per_image_path}\n"
            "See the repository README for instructions on preparing the cached "
            "viewpoint-analysis inputs."
        )

    if not per_class_path.exists():
        raise FileNotFoundError(
            f"Per-class metrics file not found: {per_class_path}"
        )
