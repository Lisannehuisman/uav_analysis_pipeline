...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create per-object multi-metric viewpoint figures from cached per-image metrics."
    )
    parser.add_argument(
        "--per-image",
    ...
    parser.add_argument(
        ...
        help="Per-class CSV used to infer class names and plotting order.",
    )
    parser.add_argument("--label", default="S0_M4", help="Model label shown in figure titles.")
    parser.add_argument(
        "--show-points",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.45,
        help="Transparency for jittered points.",
    )
    parser.add_argument(
        "--output-dir",
        default="comparison_output/object_viewpoint_metric_grid",
        help="Folder where per-object figures will be written.",
    )
...
def build_records(rows: list[dict[str, str]], class_names: list[str]) -> list[dict[str, str | float]]:
...
def grouped_values(
    ...
    for record in records:
        ...
        if np.isnan(value):
            continue
        values_by_group[str(record[group_key])].append(value)
...
def draw_group_boxplot(
    ...
    groups, data = grouped_values(records, group_key, metric)
    if not groups:
...
def create_class_grid_figure(
    ...
    fig, axes = plt.subplots(len(METRICS), 3, figsize=(16, 4.2 * len(METRICS)), sharey="row")
    group_keys = ["azimuth", "elevation", "radius"]

    for row_idx, metric in enumerate(METRICS):
        for col_idx, group_key in enumerate(group_keys):
    ...
    fig.suptitle(f"{label} viewpoint sensitivity for {class_name}", fontsize=15)
    fig.tight_layout()
...
def main() -> None:
    ...
    records = build_records(rows, class_names)
    ...
    for class_name in class_order:
        ...
        if not class_records:
            continue
        create_class_grid_figure(
            class_name=class_name,
            class_records=class_records,
            label=args.label,
            output_path=output_dir / f"{class_name}_viewpoint_metric_grid.png",
            show_points=args.show_points,
            point_alpha=args.point_alpha,
        )

    print(f"Saved per-object multi-metric viewpoint figures to: {output_dir}")
