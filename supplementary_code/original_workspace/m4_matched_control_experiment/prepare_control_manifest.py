...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Create a manifest of equal-image-count full-M4 control jobs for the best "
            "and rounded-mean single-view and pair-view budgets."
        ),
    )
    ...
    parser.add_argument(
        "--skip-single",
        action="store_true",
        help="Do not emit matched controls for the single-view best or single-view mean budgets.",
    )
    parser.add_argument(
        "--skip-pair",
        action="store_true",
        help="Do not emit matched controls for the pair-view best or pair-view mean budgets.",
    )
...
def summarize_single_mean(rows: list[dict[str, str]]) -> dict[str, object]:
    completed = [row for row in rows if row.get("evaluation_status") == "completed"]
    train_mean = statistics.mean(int(row["number_of_train_images"]) for row in completed)
    val_mean = statistics.mean(int(row["number_of_val_images"]) for row in completed)
    return {
        "source_group": "single_mean",
        "source_id": "single_mean",
        "source_label": "Mean across 72 completed single-view models",
        "train_count": round_half_up(train_mean),
        "val_count": round_half_up(val_mean),
        "reference_mAP50-95": statistics.mean(float(row["mAP50-95"]) for row in completed),
        "reference_mAP50": statistics.mean(float(row["mAP50"]) for row in completed),
        "reference_F1": statistics.mean(float(row["F1"]) for row in completed),
    }


def summarize_pair_mean(rows: list[dict[str, str]]) -> dict[str, object]:
    completed = [row for row in rows if row.get("option_a_status") == "completed"]
    train_mean = statistics.mean(int(row["number_of_train_images"]) for row in completed)
    val_mean = statistics.mean(int(row["number_of_val_images"]) for row in completed)
    return {
        "source_group": "pair_mean",
        "source_id": "pair_mean",
        "source_label": "Mean across completed pair-view models",
        "train_count": round_half_up(train_mean),
        "val_count": round_half_up(val_mean),
        "reference_mAP50-95": statistics.mean(float(row["mAP50-95"]) for row in completed),
        "reference_mAP50": statistics.mean(float(row["mAP50"]) for row in completed),
...
def main() -> None:
    ...
    if not args.skip_single:
        ...
        for seed in args.seeds:
            ...
            next_index += 1
        single_mean = summarize_single_mean(single_rows)
        for seed in args.seeds:
            manifest_rows.append(
                {
                    "control_index": next_index,
                    "control_id": f"mc_single_mean_s{seed:02d}",
                    "label": f"M4 matched to single-view mean (seed {seed})",
                    "source_group": single_mean["source_group"],
                    "source_id": single_mean["source_id"],
                    "source_label": single_mean["source_label"],
                    "train_count": single_mean["train_count"],
                    "val_count": single_mean["val_count"],
                    "seed": seed,
                    "sampling_strategy": "stratified_viewpoint",
                    "reference_mAP50-95": single_mean["reference_mAP50-95"],
                    "reference_mAP50": single_mean["reference_mAP50"],
                    "reference_F1": single_mean["reference_F1"],
                }
            )
            next_index += 1
    ...
    if not args.skip_pair:
        ...
        for seed in args.seeds:
            ...
            next_index += 1
        pair_mean = summarize_pair_mean(pair_rows)
        for seed in args.seeds:
            manifest_rows.append(
                {
                    "control_index": next_index,
                    "control_id": f"mc_pair_mean_s{seed:02d}",
                    "label": f"M4 matched to pair-view mean (seed {seed})",
                    "source_group": pair_mean["source_group"],
                    "source_id": pair_mean["source_id"],
                    "source_label": pair_mean["source_label"],
                    "train_count": pair_mean["train_count"],
                    "val_count": pair_mean["val_count"],
                    "seed": seed,
                    "sampling_strategy": "stratified_viewpoint",
                    "reference_mAP50-95": pair_mean["reference_mAP50-95"],
                    "reference_mAP50": pair_mean["reference_mAP50"],
                    "reference_F1": pair_mean["reference_F1"],
                }
            )
            next_index += 1
