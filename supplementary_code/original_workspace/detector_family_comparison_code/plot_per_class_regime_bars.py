...
def resolve_official_coco_gt(data_yaml: Path, split: str) -> Path:
    if split == "test":
        stem_candidates = [data_yaml.stem]
        for regime in REGIME_ORDER:
            if data_yaml.stem.lower().startswith(regime.lower()) and regime not in stem_candidates:
                stem_candidates.append(regime)

        for stem in stem_candidates:
            candidate = DEFAULT_OUTPUT_DIR / "standardized_test_eval" / "ground_truth" / f"{stem}_{split}_gt.json"
            if candidate.exists():
                return candidate
        raise FileNotFoundError(
            f"Missing cached test ground-truth JSON for {data_yaml.stem}. "
