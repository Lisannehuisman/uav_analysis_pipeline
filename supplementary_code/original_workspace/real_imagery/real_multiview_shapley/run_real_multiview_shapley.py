...
REAL_DIR = WORKSPACE / "real_imagery"
UAV_DIR = REAL_DIR / "UAV footage"
GT_LABEL_DIR = REAL_DIR / "test" / "labels"
DEFAULT_OUTPUT_DIR = REAL_DIR / "real_multiview_shapley" / "outputs"

DEFAULT_RUNS = {
    "synthetic_m4": WORKSPACE / "outputs" / "imported_runs" / "yolov8l_m4" / "M4_clean_yolov8l_run1" / "weights" / "best.pt",
    "real_uav_finetuned": REAL_DIR
    / "real_uav_finetune"
    / "real_uav_finetune_yolov8l_m4_lowLR"
    / "weights"
    / "last.pt",
}

# Timestamped UAV ground-truth labels in real_imagery/test/labels use this class order.
# This follows real_imagery/data.yaml, which differs from the older 10-class
# review-set order used by real_imagery/real_uav_finetune/analyze_real_uav_results.py.
GT_CLASS_NAMES = [
    "container",
    "male",
    "suv",
...
def build_report(
    ...
    lines = [
        ...
        "- Ground-truth labels for the timestamped UAV images use the 5-class order in `real_imagery/data.yaml`.",
    ...
    lines.extend(
        [
            ...
            "## Output Files",
            "",
            "- `real_multiview_manifest.csv`: auditable object/view manifest.",
