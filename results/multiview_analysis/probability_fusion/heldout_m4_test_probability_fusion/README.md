...
- Observed cached coverage: min `4`, max `18`, mean `10.80` views per instance.
- Instances with the full expected grid present in the current cache: `0` / `205`.
...
- Best multi-view summary row: `Noisy-OR` at `k=3`.
- Mean fused score: `0.9979`.
- Average precision: `1.0000`.
- Best-threshold F1: `1.0000` at threshold `0.7595`.
- Mean uplift vs best constituent single view: `+0.0456`.
- Rate of beating the best constituent single view: `0.9968`.
- Best rescue row: `Noisy-OR` at `k=3` with uplift `+0.0456`.
- Best product row: `Product` at `k=3` with uplift `-0.1590`.
...
- Strongest pairwise geometry summary row: `same_elevation_different_azimuth` with `Noisy-OR`.
- Pair-group mean uplift vs best constituent single view: `+0.0432`.
...
Generated in `C:\Users\lisan\OneDrive\Documents\New project\probability_fusion\outputs\heldout_m4_test_probability_fusion`.
